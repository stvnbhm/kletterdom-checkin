...
run_app_setup() {
    ...
    docker compose exec -T app sh -c 'mkdir -p storage/sessions storage/throttle storage/imports backups && chown -R www-data:www-data storage/sessions storage/throttle 2>/dev/null || true && chmod 777 backups storage/imports storage/sessions 2>/dev/null || true'
...
fix_storage_permissions() {
    ...
    if [[ -w storage && -w storage/imports ]]; then
        ...
        chmod 777 storage/imports storage/sessions 2>/dev/null || true
        return 0
    fi
